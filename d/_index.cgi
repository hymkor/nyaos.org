...
sub call_block{
    ...
    while( scalar(@line) > 0 ){
        foreach my $key (sort keys %::block_syntax_plugin){
            if( $::block_syntax_plugin{$key}->(\@line,$session) ){
                # &puts("&lt;$key&gt;");
                last;
            }
        }
...
sub block_midashi2{ ### !!!... ###
    my ($lines,$session)=@_;
    return 0 unless $lines->[0] =~ /\A\s*(\!{1,4})(.*)\Z/s;
...
sub block_htmltag{ ### <blockquote> or <center>
    my ($lines,$session)=@_;
    return 0 unless $lines->[0] =~ /\A\s*&lt;(blockquote|center)&gt;/si;
    my $fragment=$'; 
    my $tag=$1; 
    shift(@{$lines});
...
sub block_separator{ ### ---
    my ($lines,$session)=@_;
    return 0 unless $lines->[0] =~ /\A\s*\-\-\-+\s*\Z/;
    shift(@{$lines});
    &puts( '<hr class="sep">' );
    1;
}
