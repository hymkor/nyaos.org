    ...
    %::preferences = (
	    ...
	    { desc=>'Time Zone string(for example, JST-9 )' ,
	      name=>'TZ' , size=>6 } ,
            ...
            { desc=>'Max bytes for pagename' , 
              name=>'max_bytes_for_pagename' , size=>3 },
        ],
    );
...
HERE
    ...
    my $maxbytes = $::config{max_bytes_for_pagename} || 240;
    @::index_columns = (
        ...
        sub{ 
            my $result=( length($_[0]->{fname}) >= $maxbytes 
                       ? "<blink>(TOO LONG NAME)</blink>" : "");
            for my $name (keys %{$_[0]->{attach}}){
                if( length($_[0]->{attach}->{$name}) >= $maxbytes ){
                    $result = "<blink>(TOO LONG NAME)</blink>";
                }
            }
            $result;
        }
